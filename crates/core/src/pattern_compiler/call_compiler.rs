...
impl NodeCompiler for CallCompiler {
    ...
    fn from_node(
        ...
        let named_args = node_to_args_pairs(named_args, context.lang, kind, &expected_params)?;
...
impl NodeCompiler for PrCallCompiler {
    ...
    fn from_node(
        ...
        let named_args = node.named_children_by_field_name("named_args");
        let named_args = node_to_args_pairs(named_args, context.lang, name, &expected_params)?;
...
fn named_args_to_hash_map(
    ...
    named_args
        .into_iter()
        .map(|(name, node)| {
            let name = context.lang.metavariable_prefix().to_owned() + &name;
            let pattern = PatternCompiler::from_node(node, &mut context.with_rhs(true))?;
            Ok((name, pattern))
        })
        .collect()
}
