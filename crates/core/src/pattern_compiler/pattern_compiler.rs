...
impl PatternCompiler {
    ...
    pub(crate) fn from_snippet_node(
        ...
        let ranges = metavariable_ranges(&node, context.lang);
        ...
        fn node_to_astnode(
            ...
            let node_types = context.lang.node_types();
            let metavariable = metavariable_descendent(&node, context_range, range_map, context)?;
            ...
            let args =
                fields
                    ...
                    .filter(|field| {
                        ...
                        !((node.node.child_by_field_id(field.id()).is_none() && context.lang.optional_empty_field_compilation(sort, field.id()))
                        ...
                        //     }
                        // }
                        || context.lang.skip_snippet_compilation_of_field(sort, field.id()))
                    })
                    .map(|field| {
                        let field_id = field.id();
                        let mut nodes_list = node
                            .named_children_by_field_id(field_id)
                            .map(|n| node_to_astnode(n, context_range, range_map, context))
        ...
        node_to_astnode(node, context_range, &range_map, context)
    }
}
...
impl NodeCompiler for PatternCompiler {
    ...
    fn from_node(
        ...
        match kind.as_ref() {
            ...
            "nodeLike" => CallCompiler::from_node(node, context),
            "list" => Ok(Pattern::List(Box::new(ListCompiler::from_node(
                node, context,
            )?))),
            "listIndex" => Ok(Pattern::ListIndex(Box::new(ListIndexCompiler::from_node(
                node, context,
            )?))),
            "map" => Ok(Pattern::Map(Box::new(MapCompiler::from_node(
                node, context,
            )?))),
            ...
            "regexPattern" => Ok(Pattern::Regex(Box::new(RegexCompiler::from_node(
                node, context,
            )?))),
            "variable" => Ok(Pattern::Variable(VariableCompiler::from_node(
                node, context,
            )?)),
            "codeSnippet" => CodeSnippetCompiler::from_node(node, context),
...
fn implicit_metavariable_regex(
    ...
    let variable_regex = context.lang.replaced_metavariable_regex();
...
fn metavariable_descendent(
    ...
    loop {
        ...
        if is_metavariable(&node, context.lang) {
            let name = node.text();
            if is_reserved_metavariable(name.trim(), Some(context.lang)) && !context.is_rhs {
                bail!("{} is a reserved metavariable name. For more information, check out the docs at https://docs.grit.io/language/patterns#metavariables.", name.trim_start_matches(context.lang.metavariable_prefix_substitute()));
            }
            let range: Range = node.range().into();
...
fn metavariable_ranges(node: &NodeWithSource, lang: &impl Language) -> Vec<Range> {
    ...
    traverse(CursorWrapper::new(cursor, node.source), Order::Pre)
        .flat_map(|n| {
            let child = NodeWithSource::new(n.node, node.source);
