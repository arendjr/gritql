...
impl NodeCompiler for LanguageSpecificSnippetCompiler {
    ...
    fn from_node(
        ...
        parse_snippet_content(content, range, context)
    }
}
...
pub(crate) fn dynamic_snippet_from_source(
    ...
    let mut metavariables = split_snippet(source, context.lang);
    ...
    for (byte_range, var) in metavariables {
        ...
        if let Some(var) = context.vars.get(&var.to_string()) {
            ...
            if context.file == DEFAULT_FILE_NAME {
...
pub(crate) fn parse_snippet_content(
    ...
    // identifier.
    if context.lang.metavariable_bracket_regex().is_match(source) {
        if context.is_rhs {
            Ok(Pattern::Dynamic(
        ...
        if context.lang.exact_variable_regex().is_match(source.trim()) {
            match source.trim() {
        ...
        let snippet_trees = context.lang.parse_snippet_contexts(source);
        ...
        let snippet_patterns: Vec<(SortId, Pattern)> = snippet_nodes
            ...
            .map(|node| {
                Ok((
                    node.node.kind_id(),
                    PatternCompiler::from_snippet_node(node, range, context)?,
                ))
            })
